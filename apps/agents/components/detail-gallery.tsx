...
export function DetailGallery({ images, alt }: Props) {
    ...
    ? createPortal(
        ...
        >
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ position: "relative", maxWidth: "90vw", maxHeight: "80vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            <img src={images[index]} alt={`${alt} - ${index + 1}`} style={{ maxWidth: "90vw", maxHeight: "80vh", objectFit: "contain", borderRadius: 8 }} />
          </div>
          {images.length > 1 && (
            <>
              <button onClick={(e) => { e.stopPropagation(); prev(); }} style={{ position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)", background: "rgba(255,255,255,0.15)", border: "none", color: "#fff", width: 48, height: 48, borderRadius: "50%", fontSize: 24, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }} aria-label="Previous">‹</button>
              <button onClick={(e) => { e.stopPropagation(); next(); }} style={{ position: "absolute", right: 16, top: "50%", transform: "translateY(-50%)", background: "rgba(255,255,255,0.15)", border: "none", color: "#fff", width: 48, height: 48, borderRadius: "50%", fontSize: 24, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }} aria-label="Next">›</button>
            </>
          )}
          <div onClick={(e) => e.stopPropagation()} style={{ display: "flex", gap: 6, overflowX: "auto", marginTop: 16, padding: "0 20px", maxWidth: "90vw" }}>
            {images.map((src, i) => (
              // eslint-disable-next-line @next/next/no-img-element
              <img key={i} src={src} alt={`Thumb ${i + 1}`} onClick={() => setIndex(i)}
                style={{ flexShrink: 0, width: 72, height: 52, borderRadius: 6, cursor: "pointer", objectFit: "cover", border: i === index ? "2px solid #fff" : "2px solid transparent", opacity: i === index ? 1 : 0.5, transition: "opacity 0.2s, border-color 0.2s" }} />
            ))}
          </div>
        </div>,
        document.body,
  ...
  return (
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 6, borderRadius: 12, overflow: "hidden", aspectRatio: "16/7", maxHeight: 520, cursor: "pointer" }}>
      ...
      {images.length > 3 && (
        <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingTop: 10, paddingBottom: 4 }}>
          {images.map((src, i) => (
            // eslint-disable-next-line @next/next/no-img-element
            <img key={i} src={src} alt={`Thumb ${i + 1}`} onClick={() => openAt(i)}
              style={{ flexShrink: 0, width: 80, height: 56, borderRadius: 8, cursor: "pointer", objectFit: "cover", border: "2px solid var(--color-border)", transition: "border-color 0.15s" }} />
          ))}
        </div>
      )}
      {lightbox}
